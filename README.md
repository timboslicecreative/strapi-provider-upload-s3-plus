...
A simple upload provider using aws-s3 allowing the use of ACL and folders on any aws-s3 compatible S3 service.
...
**Note:** By default the `apiVersion` is "2006-03-01", you can change this using the `apiVersion` property in `providerOptions`. 
```
module.exports = ({env}) => ({
    upload: {
        provider: "aws-s3-plus",
